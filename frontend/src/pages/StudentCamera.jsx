...
export default function FastFocusTracker({ sessionId, studentUUID, slideIndex }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [bf, md] = await Promise.all([
        blazeface.load(),
        fld.createDetector(fld.SupportedModels.MediaPipeFaceMesh, { runtime: 'tfjs', maxFaces: 1 })
      ]);
      setBlaze(bf);
      setDetector(md);
    ...
    async function detect() {
      ...
      if (faces.length === 0) {
        ...
        if (mesh && mesh.keypoints) {
          // nose
          const nose = mesh.keypoints.find(k => k.name === 'noseTip') || mesh.keypoints[1];
          // center of this box
  ...
  return (
    <div className="p-2 bg-gray-100 rounded-md">
      <video ref={videoRef} className="w-48 h-36 rounded border mb-2" muted playsInline />
